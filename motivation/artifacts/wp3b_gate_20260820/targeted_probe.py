...
def _run_checkpoint_control(scheduler, request: dict) -> dict:
    ...
    if action not in ("inspect", "evict_mamba", "evict_mamba_only"):
    ...
    if action in ("evict_mamba", "evict_mamba_only"):
        ...
        if action == "evict_mamba" and (
            path["n_children"] < 1 or path["is_device_leaf"]
        ):
            raise AssertionError(
    ...
    if action in ("evict_mamba", "evict_mamba_only"):
        ...
        try:
            ...
            if action == "evict_mamba":
                ...
                # Experimental intervention for WP3B:
                # tombstone only the target Mamba device state.
                # Do NOT apply leaf priority collapse/cascade semantics.
